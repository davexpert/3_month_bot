...
async def random_profile_call_2(call: types.CallbackQuery):
    db = Database()
    profiles = db.sql_select_dis_filter_profiles(
        tg_id=call.from_user.id
    )
    if profiles:
        random_profile = random.choice(profiles)
        ...
        await bot.send_message(
            chat_id=call.from_user.id,
            text='There is no profiles that u have not disliked'
        )
...
async def dislike_detect_call(call: types.CallbackQuery):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=call.from_user.id,
            text='U disliked this profile before'
        )
    finally:
        await random_profile_call_2(call=call)
...
def register_profile_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        random_profile_call_2,
        lambda call: call.data == 'random_profile2'
    )
    dp.register_callback_query_handler(
        delete_profile_call,
